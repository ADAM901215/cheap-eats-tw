...
-- ------------------------------------------------------------
-- 資料表：user_points（個人任務／積分，用來做「省錢成就」儀表板與兌換商店）
-- ------------------------------------------------------------
create table if not exists public.user_points (
  user_id uuid primary key,
  points int not null default 0,
  reports_count int not null default 0,
  photo_reports_count int not null default 0,
...
-- insert/update 都交給下面的 security definer 函式處理，一般使用者不能直接寫入，避免自己加分作弊
...
-- ------------------------------------------------------------
-- Trigger：新增一筆 restaurants 回報 → 自動加分
-- 一般回報 +30 分，附照片再多 +20 分（鼓勵大家附證明照片）
-- ------------------------------------------------------------
create or replace function public.handle_new_restaurant_points()
returns trigger as $$
declare
  bonus int := case when new.photo_url is not null then 20 else 0 end;
begin
  insert into public.user_points(user_id, points, reports_count, photo_reports_count)
  values (new.reporter_id, 30 + bonus, 1, case when new.photo_url is not null then 1 else 0 end)
  on conflict (user_id) do update set
    points = public.user_points.points + 30 + bonus,
    reports_count = public.user_points.reports_count + 1,
    photo_reports_count = public.user_points.photo_reports_count + (case when new.photo_url is not null then 1 else 0 end),
    updated_at = now();
  return new;
end;
$$ language plpgsql security definer;

drop trigger if exists on_restaurant_created_points on public.restaurants;
create trigger on_restaurant_created_points
  after insert on public.restaurants
  for each row execute function public.handle_new_restaurant_points();

-- ------------------------------------------------------------
-- 函式：每日簽到（+10 分，連續簽到會累計 streak，一天只能簽一次）
-- ------------------------------------------------------------
create or replace function public.daily_checkin()
returns table(success boolean, points int, streak int) as $$
declare
  rec public.user_points%rowtype;
  today date := (now() at time zone 'Asia/Taipei')::date;
  new_streak int;
begin
  select * into rec from public.user_points where user_id = auth.uid() for update;

  if not found then
    insert into public.user_points(user_id, points, checkin_streak, last_checkin_date)
    values (auth.uid(), 10, 1, today);
    return query select true, 10, 1;
    return;
  end if;

  if rec.last_checkin_date = today then
    return query select false, rec.points, rec.checkin_streak;
    return;
  end if;

  if rec.last_checkin_date = today - 1 then
    new_streak := rec.checkin_streak + 1;
  else
    new_streak := 1;
  end if;

  update public.user_points
    set points = points + 10, checkin_streak = new_streak, last_checkin_date = today, updated_at = now()
    where user_id = auth.uid();

  return query select true, rec.points + 10, new_streak;
end;
$$ language plpgsql security definer;
...
-- ------------------------------------------------------------
-- 函式：兌換積分（原子性檢查夠不夠分再扣，避免灌水／作弊）
-- ------------------------------------------------------------
create or replace function public.redeem_points(p_cost int)
returns table(success boolean, new_points int) as $$
declare
  current_points int;
begin
  select points into current_points from public.user_points where user_id = auth.uid() for update;
  if current_points is null then current_points := 0; end if;

  if current_points < p_cost then
    return query select false, current_points;
  else
    update public.user_points set points = points - p_cost, updated_at = now() where user_id = auth.uid();
    return query select true, current_points - p_cost;
  end if;
end;
$$ language plpgsql security definer;
...
-- ============================================================
-- 完成！可以到左側「Table Editor」確認資料表都建立好了。
-- ============================================================
