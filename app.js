(function(){
  ...
  let map, markerLayer, pickMap, pickMarker;
  ...
  function initMap(){
    map = L.map('map', { zoomControl:true }).setView([25.0478,121.5319], 13);
  ...
  function makeIcon(r){
    ...
    return L.divIcon({
      className:'',
      html:`<div class="pin-badge" style="background:${t.color}"><span>$${r.price}</span></div>`,
  ...
  function locateOnMap(id){
    ...
    setTimeout(()=>{
      markerLayer.eachLayer(m=>{ if(m.restaurantId===id) m.openPopup(); });
    }, 650);
  }
  ...
  document.getElementById('locateBtn').addEventListener('click', function(){
    ...
    navigator.geolocation.getCurrentPosition(
      pos=>{ map.flyTo([pos.coords.latitude,pos.coords.longitude], 15, {duration:0.6}); },
      ()=>{ showToast('無法取得你的位置'); }
    );
  });
